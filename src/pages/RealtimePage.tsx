...
import { configureOrtForCdn, ORT_WASM_CDN_BASE } from "../realtime/ortAssets";
...
> = {
  ...
  multilingual: {
    label: "다국어 민감 모드",
    description:
      "짧고 익숙하지 않은 비영어권 발화 — VAD 임계값 완화 + 짧은 misfire도 STT로 복구",
    profile: {
...
function RealtimePage() {
  ...
  const processChunkOnServer = async (
    ...
    // API 는 항상 서버 루트 (/api/...) 로 나간다 — 정적 자산 base 와 분리.
    // dev / preview / 배포 모두 "/api/119/realtime/process" 로 동일하며,
    // 브라우저가 현재 origin 을 자동 prefix 한다.
  ...
  const handleStart = async () => {
    ...
    try {
      const vad = await MicVAD.new({
        ...
        // ort wasm/.mjs 는 Vite dev 의 /public dynamic-import 차단을 피하려
        // CDN 절대 URL 에서 로드한다. 모델/worklet 은 baseAssetPath(/static/119/vad/)
        // 에서 fetch — 동적 import 가 아니라 안전. 자세한 배경은
