...
마운트 위치: OpenWebUI 컨테이너 안 `<owi-root>/routers/ems_realtime.py`
...
# ============================================================================
# 기본값은 공개 저장소용 예시 placeholder 다. 운영에서는 환경변수로 override 한다.
# OWI 내부 라우터(/api/v1/audio/*)는 호출하지 않는다 — 자기 자신을 재귀 호출하는
# 위험을 피하기 위해 외부 게이트웨이로 직접 나간다.

STT_URL = os.getenv(
    "EMS_STT_URL",
    "https://stt-gateway.example.internal/v1/audio/transcriptions",
)
STT_MODEL = os.getenv("EMS_STT_MODEL", "cohere-transcribe")

DIARIZE_URL = os.getenv(
    "EMS_DIARIZE_URL",
    # nginx 등 운영 게이트웨이 경유 권장. 사설 IP 직접 사용 금지.
    # m4a 도 그대로 전송 가능하며 file 만 필수, language/model/response_format 은 선택.
    "https://diarize-gateway.example.internal/v1/audio/transcriptions",
)

LLM_URL = os.getenv(
    "EMS_LLM_URL",
    # OpenAI-compatible chat/completions 엔드포인트. 인증/네트워크 정책은
    # 운영 환경에서 env 와 nginx 로 설정한다.
    "https://llm-gateway.example.internal/v1/chat/completions",
)
# 실제 사용할 모델은 env override(EMS_LLM_MODEL=...) 로 지정한다.
LLM_MODEL = os.getenv("EMS_LLM_MODEL", "example-llm-model")

# LLM API key 조회 우선순위:
#   1) EMS_LLM_API_KEY (env)
#   2) JB_LLM_API_KEY  (env)
#   3) OPENAI_API_KEY  (env)
#   4) EMS_LLM_API_KEY_FILE 경로의 파일 내용 (env 가 컨테이너 부팅 스크립트로
#      덮어쓰이는 환경을 위한 폴백). 경로 기본값은 `/run/secrets/ems_llm_api_key`
#      이며 환경에 맞게 EMS_LLM_API_KEY_FILE 로 override 한다.
#
# 절대 코드에 하드코딩하지 않으며, 키 값 자체는 어떤 경우에도 로그에 출력하지 않는다.

LLM_API_KEY_FILE = os.getenv("EMS_LLM_API_KEY_FILE", "/run/secrets/ems_llm_api_key")
...
TTS_URL = os.getenv(
    "EMS_TTS_URL",
    "https://tts-gateway.example.internal/v1/audio/speech",
)
TTS_MODEL = os.getenv("EMS_TTS_MODEL", "example-tts-model")
...
# 사설/자체 서명 인증서 환경(컨테이너 ↔ 내부 게이트웨이)에서 핸드셰이크 실패를
# 피하기 위해 SSL 검증을 끌 수 있도록 환경변수로 제어한다. 기본값은 "false"(검증 끔).
# 정식 인증서가 붙은 운영 환경에서는 EMS_SSL_VERIFY=true 로 켜는 것을 권장.
...
# 운영 게이트웨이가 same-origin/cookie 또는 네트워크 ACL 로 자체 인증을 수행하는
# 환경에서는 EMS_LLM_AUTH_BYPASS=true 로 명시해 Authorization 헤더를 미부착한다.
# 명시되지 않은 환경에서는 항상 키를 부착(또는 EMS_LLM_AUTH_REQUIRED=true 시 차단).
_LLM_AUTH_BYPASS = _parse_bool(os.getenv("EMS_LLM_AUTH_BYPASS", "false"), False)
if _LLM_AUTH_BYPASS:
    log.info(
        "[ems_realtime] LLM_AUTH=bypassed (EMS_LLM_AUTH_BYPASS=true — "
        "운영 게이트웨이 자체 인증 가정, Authorization 미부착)"
    )
elif LLM_API_KEY:
    log.info(
...
# 기본 1 — 1단어 발화도 즉시 commit 한다. 119 신고자가 단발로 외친 후 침묵하는
# 케이스("Help" / "물" / "네") 가 pending 에 갇혀 인식되지 않는 회귀를 막기 위함.
# 영어 토막 STT 결과("said it" / "you know" 등) 는 별도 INCOMPLETE_EN_FRAGMENTS
# 셋 매칭으로 여전히 fragment 처리되므로 보호된다. 더 보수적으로 가려면
# env 로 EMS_REALTIME_PENDING_MIN_WORDS=2 또는 3 으로 다시 올릴 수 있다.
EMS_REALTIME_PENDING_MIN_WORDS = int(
    os.getenv("EMS_REALTIME_PENDING_MIN_WORDS", "1")
)
...
# 보낼지 여부. Phase B 부터 기본 true — confirmed_caller_language 가 잠겼을 때 게이트웨이
# 디코더 prior 로 짧은 chunk 의 인식 품질을 끌어올린다. 잘못된 hint 는 게이트웨이가
# silent fallback 처리하므로 강제 오인식 위험은 없다.
EMS_STT_USE_LANGUAGE_HINT: bool = _parse_bool(
    os.getenv("EMS_STT_USE_LANGUAGE_HINT", "true"), True
)
...
class SessionState:
    ...
    __slots__ = (
        ...
        "pending_started_at",
        "pending_lang",
        # ---- Phase B: caller language confirmation ----
        # 같은 외국어 caller 발화가 2회 연속 등장하면 confirmed_caller_language 로 잠금.
        # 한국어 caller 는 confirmation stream 에서 제외 (stream 을 끊지도 카운트하지도 않음).
        # 첫 버전에선 잠금 해제 없음 — 게이트웨이 silent fallback 이 안전망.
        "confirmed_caller_language",
        "_prev_foreign_caller_language",
        # ---- Phase C: 콘솔 드롭다운 수동 잠금 (매 호출 client 가 주입) ----
        # process_chunk 진입부에서 form 의 manual_language_lock 으로 set/clear 한다.
        # 활성 시: STT 힌트 최우선, register_caller_language 의 모든 mutation 차단.
        # None 으로 클리어되면: 자동 confirm 흐름이 다시 작동한다 (confirmed 도 같이 클리어).
        "manual_locked",
    )
    ...
    def __init__(self) -> None:
        ...
        # Phase B confirmation — 잠금 전엔 둘 다 None.
        self.confirmed_caller_language: Optional[str] = None
        self._prev_foreign_caller_language: Optional[str] = None
        # Phase C — 콘솔 수동 잠금. client 가 매 호출 form 으로 갱신/클리어.
        self.manual_locked: Optional[str] = None

    def register_caller_language(self, lang: str) -> None:
        # Phase C — 콘솔 수동 잠금 활성 시 STT detected lang 이 세션 상태를 못 흔들게 차단.
        # 이 함수의 모든 부수효과(primary/latest/confirmed/secondary/has_korean_caller/
        # _prev_foreign_caller_language 갱신, auto-confirm 로그)는 모두 언어 상태 mutation
        # 이므로 블록 단위 early-return 이 적절 범위다.
        if self.manual_locked is not None:
            return
        ...
        # ---- Phase B: caller language confirmation stream ----
        # 한국어 caller 는 stream 에서 제외 (끊지도, 카운트하지도 않음).
        if lang == "ko":
            return
        # 이미 확정된 세션은 더 이상 갱신하지 않는다 (잠금 해제 없음).
        if self.confirmed_caller_language is not None:
            return
        if self._prev_foreign_caller_language == lang:
            self.confirmed_caller_language = lang
            log.info(
                "[ems_realtime] caller_language_confirmed lang=%s", lang,
            )
        else:
            self._prev_foreign_caller_language = lang
...
def is_hallucination(text: str) -> bool:
    ...
    if n in HALLUCINATIONS:
        return True
    # 과거에는 len(n) <= 2 도 자동 환각으로 폐기했으나, 신고자의 짧은 응답("네"/"예"/"응"/
    # "Hi"/"OK"/"No") 까지 묶여 사라지는 회귀가 있어 제거. 환각 폐기는 HALLUCINATIONS
    # 셋의 명시적 매칭에만 의존한다 — 잡음에 대한 STT 응답은 대부분 이 셋 또는 빈 결과로
    # 떨어지므로 안전.
    return False
...
def is_incomplete_fragment(text: str, lang: str) -> bool:
    ...
    # 5) 과거에는 "단어 < 5 + 종결 없음" 도 fragment 로 잡았으나, 119 신고자가 짧게
    # 외치는 정상 발화("도와주세요 빨리", "Help me", "여기 학교 앞") 가 무조건 fragment 로
    # 묶이는 회귀를 만들어 제거. fragment 판정은 응급 키워드 미매칭 + 명령형 아님 +
    # 단어 < PENDING_MIN_WORDS + 종결부호 없음 (= 위 1~4 통과) 인 케이스에만 적용된다.
    return False
...
def classify_speaker(text: str, lang: str, sess: SessionState) -> dict:
    ...
    target_for_op = (
        sess.confirmed_caller_language
        or sess.latest_caller_language
    ...
    if lang == "ko":
        # 양쪽 표현이 모두 매칭되는 모호 케이스 — 결과를 뒤집기보다 신뢰도를 낮춘다.
        if op_score >= 1 and cl_score >= 1:
            diff = abs(op_score - cl_score)
            if cl_score > op_score:
                return {
                    "speaker": "caller",
                    "confidence": 0.45 if diff == 1 else 0.55,
                    "reason": (
                        f'한국어 — caller/operator 표현 혼재(낮은 확신): '
                        f'caller "{caller_hits[0]}" vs operator "{operator_hits[0]}"'
                    ),
                    "source_language": "ko",
            ...
            if op_score > cl_score:
                return {
                    "speaker": "operator",
                    "confidence": 0.50 if diff == 1 else 0.60,
                    "reason": (
                        f'한국어 — operator/caller 표현 혼재(낮은 확신): '
                        f'operator "{operator_hits[0]}" vs caller "{caller_hits[0]}"'
                    ),
                    "source_language": "ko",
                    "target_language": target_for_op,
                }
            return {
                ...
                "confidence": 0.30,
                "reason": (
                    f'한국어 — operator/caller 표현 동시 매칭, 모호: '
                    f'operator "{operator_hits[0]}" vs caller "{caller_hits[0]}"'
                ),
                "source_language": "ko",
                "target_language": "unknown",
...
async def call_translate(
    ...
    # LLM 호출 규약: OpenAI-compatible POST {EMS_LLM_URL} chat/completions.
    # 모델은 env(EMS_LLM_MODEL) 로 지정. 인증 정책:
    #   - EMS_LLM_AUTH_BYPASS=true (운영 게이트웨이 자체 인증) → Authorization 미부착.
    #   - 그 외:
    #       · LLM_API_KEY 있음                              → Bearer 헤더 부여
    #       · LLM_API_KEY 없음 + LLM_AUTH_REQUIRED=true     → LLMAuthMissing raise
    #       · LLM_API_KEY 없음 + LLM_AUTH_REQUIRED=false    → 헤더 없이 호출
    # 키 조회는 _resolve_llm_api_key (env + 파일 폴백). 키 값은 로그에 출력하지 않는다.
    auth_bypass = _LLM_AUTH_BYPASS
    if not auth_bypass and not LLM_API_KEY and LLM_AUTH_REQUIRED:
        log.warning("LLM API key missing (EMS_LLM_AUTH_REQUIRED=true)")
        raise LLMAuthMissing("LLM API key missing")

    headers: dict[str, str] = {"Content-Type": "application/json"}
    auth_attached = False
    if not auth_bypass and LLM_API_KEY:
        headers["Authorization"] = f"Bearer {LLM_API_KEY}"
        auth_attached = True

    # 호출 시점 인증 상태 진단 — token 값은 절대 출력 금지, attached 여부/길이만 노출.
    log.info(
        "[ems_realtime] LLM call auth_header_attached=%s token_length=%d "
        "auth_required=%s auth_bypass=%s model=%s",
        auth_attached,
...
async def _process_diarization_segments(
    ...
    for seg in segments:
        ...
        # Phase B: caller 로 분류된 외국어 segment 의 source_language 가 confirmed 와
        # 다르면 confirmed 로 보정. classify 입력(seg_lang) 은 그대로 유지.
        # 한국어 caller(src=ko) 는 제외.
        if (
            role == "caller"
            and sess.confirmed_caller_language
            and src != "ko"
            and src != sess.confirmed_caller_language
        ):
            src = sess.confirmed_caller_language
...
def _envelope(
    ...
    *,
    ...
    # Phase B: 세션의 잠금된 caller 언어. None 이면 아직 확정 전. 프론트가 즉시 표시할
    # 의무는 없고, 향후 디버깅 / UI 인디케이터용으로 envelope 에 노출만 한다.
    confirmed_caller_language: Optional[str] = None,
) -> dict:
    return {
        ...
        "confirmed_caller_language": confirmed_caller_language,
    }
...
async def process_chunk(
    ...
    # Phase C — 콘솔 드롭다운 수동 잠금. previous_caller_language(자동/감지) 와 분리된
    # 별도 폼 필드 — 매 호출 client 가 보내며, 유효 시 세션 언어 상태를 권위적으로 덮어쓴다.
    # 미전송(None)이면 자동 감지 모드 — 기존 동작 유지 + 잔존 manual_locked / confirmed 클리어.
    manual_language_lock: Optional[str] = Form(None),
):
    ...
    _diag(
        ...
        manual_lock=manual_language_lock,
        sess_manual_locked=sess.manual_locked,
    ...
    # Phase C — 콘솔 드롭다운 수동 잠금이 들어오면 매 호출 권위적으로 세션 언어 상태를
    # 덮어쓴다. STT detected lang 의 자기예언식 드리프트로부터 보호하기 위함.
    # 자동 모드(client 가 manual_language_lock 미전송) 면 잔존 manual_locked + confirmed
    # 를 클리어해 자동 confirm 흐름이 다시 작동하게 한 뒤, 기존 부트스트랩 로직을 그대로 탄다.
    if manual_language_lock and manual_language_lock in LANG_CODES:
        prev_locked = sess.manual_locked
        sess.manual_locked = manual_language_lock
        sess.latest_caller_language = manual_language_lock
        sess.primary_caller_language = manual_language_lock
        sess.confirmed_caller_language = manual_language_lock
        if manual_language_lock == "ko":
            sess.has_korean_caller = True
        if prev_locked != manual_language_lock:
            _diag(
                "session_manual_lock_set",
                session=session_id,
                seq=client_seq,
                manual_lock=manual_language_lock,
        ...
        # 자동 모드 — 잔존 수동 잠금 해제. confirmed 도 같이 클리어해 자동 confirm 재개.
        if sess.manual_locked is not None:
            _diag(
                "session_manual_lock_clear",
                session=session_id,
                seq=client_seq,
                prev_locked=sess.manual_locked,
            )
            sess.manual_locked = None
            sess.confirmed_caller_language = None
            sess._prev_foreign_caller_language = None
    ...
    try:
        ...
        async with httpx.AsyncClient(verify=SSL_VERIFY) as client:
            ...
            if mode != "diarization" and EMS_STT_USE_LANGUAGE_HINT:
                # 우선순위: manual_locked(콘솔 드롭다운 수동 잠금) >
                #         > confirmed_caller_language(자동 잠금) > pending_lang(연속 발화 추정)
                #         > latest_caller_language.
                # manual_locked 가 있으면 4.3 의 process_chunk 진입부에서 latest/primary/
                # confirmed 까지 같은 값으로 동기시키므로 사실상 redundant 지만 명시성을 위해
                # 첫 자리 유지.
                stt_language_hint = (
                    sess.manual_locked
                    or sess.confirmed_caller_language
                    or sess.pending_lang
                    or sess.latest_caller_language
                )
            ...
            if mode == "diarization" and segments:
                ...
                return JSONResponse(
                    _envelope(
                        ...
                        confirmed_caller_language=sess.confirmed_caller_language,
                    ),
                    status_code=200,
                )
            ...
            # Phase B: caller 로 분류된 외국어 발화의 source_language 가 confirmed 와
            # 다르면 confirmed 로 보정. classify 의 입력(lang) 은 손대지 않는다
            # (operator/caller 판정 흐름 보호). 한국어 caller(source=ko) 는 제외.
            if (
                speaker == "caller"
                and sess.confirmed_caller_language
                and source_language != "ko"
                and source_language != sess.confirmed_caller_language
            ):
                source_language = sess.confirmed_caller_language
...
def get_session_info(session_id: str) -> dict:
    ...
    return {
        ...
        "confirmed_caller_language": sess.confirmed_caller_language,
        "manual_locked": sess.manual_locked,
